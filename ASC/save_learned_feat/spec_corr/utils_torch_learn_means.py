...
def enframe(x, winlen, hoplen):
    ...
    x = np.pad(x, int(winlen//2), mode='reflect') # to make windowing centred
...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self,audio_path,csv_path,file_type):
        ...
        self.winlen = 1102 # 25 ms
        self.hoplen = 441  # 10 ms
    ...
    def __getitem__(self, idx):
        ...
        x1 = data_frame[:,:self.num_time_frame]
        x2 = data_frame[:, self.num_time_frame: self.num_time_frame + self.num_time_frame]
        X.append(x1)
        X.append(x2)
        X = torch.tensor(X)
        target1 = self.labels[idx]
        target2 = self.labels[idx]
        Y.append(target1)
        Y.append(target2)
        #Y = torch.tensor(Y)
